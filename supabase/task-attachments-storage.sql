-- Run this once to support downloadable task attachments.
...
drop policy if exists "authenticated can update task attachments" on storage.objects;
...
create policy "authenticated can update task attachments"
on storage.objects
for update
to authenticated
using (bucket_id = 'task-attachments')
